<script setup lang="ts">
import { NPopover } from 'naive-ui';

const props = defineProps<{
  label: string;
  tip?: string;
}>();
</script>

<template>
  <span class="inline-flex items-center">
    {{ props.label }}
    <NPopover v-if="props.tip" trigger="hover" placement="top">
      <template #trigger>
        <span class="ml-4px mt-2px flex cursor-help items-center text-gray-400 transition-colors hover:text-primary">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M11 18h2v-2h-2v2m1-16A10 10 0 0 0 2 12a10 10 0 0 0 10 10a10 10 0 0 0 10-10A10 10 0 0 0 12 2m0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8s8 3.59 8 8s-3.59 8-8 8m0-14a4 4 0 0 0-4 4h2a2 2 0 0 1 2-2a2 2 0 0 1 2 2c0 2-3 1.75-3 5h2c0-2.25 3-2.5 3-5a4 4 0 0 0-4-4Z"
            />
          </svg>
        </span>
      </template>
      <div style="max-width: 300px; white-space: pre-wrap">{{ props.tip }}</div>
    </NPopover>
  </span>
</template>
